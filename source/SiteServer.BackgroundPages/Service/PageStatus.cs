...
namespace SiteServer.BackgroundPages.Service
{
    public class PageStatus : BasePage
    {
		...
		public void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                LtlMessage.Text = GetMessage(true, "Scheduler.exe 服务组件已启动并处于正常运行状态");
            ...
            {
                LtlMessage.Text = GetMessage(false, "Scheduler.exe 服务组件未启动");
        ...
        private string GetMessage(bool isOk, string text)
        {
            ...
            return $@"<div class=""alert alert-danger"">
                  ...
                  <p style=""margin: 10px 0 10px; font-size: 14px;"">
                    Scheduler.exe 服务组件未启动，请在WCM系统根目录下双击运行Scheduler.exe程序启用服务。
                  </p>
                </div>";
        }
