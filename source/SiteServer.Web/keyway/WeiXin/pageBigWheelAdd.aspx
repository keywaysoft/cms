<%@ Page Language="C#" Inherits="SiteServer.BackgroundPages.WeiXin.PageBigWheelAdd" %>
...
<form class="form-inline" runat="server">
  ...
  <script type="text/javascript" src="background_bigWheelAdd.js"></script>
  <style type="text/css">
  div.step {
  font-weight: bold;
  font-size: 16px;
    ...
    <div class="popover-content">
      ...
      <asp:PlaceHolder id="PhStep1" runat="server">
        <div class="row-fluid">
          ...
          <div class="Span6">
            <div class="step">第一步：配置大转盘开始属性</div>
            <table class="table noborder table-hover">
              ...
              <tr>
                <td>大转盘状态：</td>
                <td class="checkbox">
                  <asp:CheckBox id="CbIsEnabled" runat="server" checked="true" text="启用大转盘" />
                </td>
              </tr>
              <tr>
                <td>触发关键词：</td>
      ...
      <asp:PlaceHolder id="PhStep2" visible="false" runat="server">
        <div class="row-fluid">
          ...
          <div class="Span6">
            <div class="step">第二步：配置大转盘详情页</div>
            <table class="table noborder table-hover">
              <tr>
                <td width="120">大转盘活动规则：</td>
                <td>
...
3.本活动组织方拥有最终解释权" />
              ...
              <tr>
                ...
                <td>
                  ...
                  <span>0代表不限制，必须小于每人最多允许抽奖次数</span>
                </td>
              </tr>
              <tr>
                <td>活动方兑奖密码：</td>
          ...
          <div class="Span6">
            <div class="intro-grid">
              <p><strong>大转盘图片：</strong></p>
              <p><span>不会设计转盘的请留空，系统自动识别奖品数量并调用默认的转盘。</span></p>
              <p><span>需要自己制作转盘图片的，请<a href="../../sitefiles/services/weixin/lottery/imgBigWheel/bigWheel.zip" target="_blank">下载转盘模版文件</a>，修改后上传，格式为png透明图片。</span></p>
              <asp:Literal id="LtlContentAwardImageUrl" runat="server" />
              <a id="js_contentAwardImageUrl" href="javascript:;" onclick="return false;" class="btn btn-success">上传</a>
            </div>
            <div class="intro-grid">
        ...
        new AjaxUpload('js_contentAwardImageUrl', {
         action: '<%=GetUploadUrl()%>',
         name: "Upload",
         data: {},
         onSubmit: function(file, ext) {
         ...
         onComplete: function(file, response) {
           if (response) {
             response = eval("(" + response + ")");
             if (response.success == 'true') {
               $('#preview_contentAwardImageUrl').attr('src', response.url);
               $('#contentAwardImageUrl').val(response.virtualUrl);
             } else {
               alert(response.message);
             }
           }
      ...
      <asp:PlaceHolder id="PhStep5" visible="false" runat="server">
        <div class="row-fluid">
          ...
          <div class="Span6">
            <div class="step">第五步：配置大转盘结束属性</div>
            <table class="table noborder table-hover">
              <tr>
                <td width="120">大转盘结束主题：</td>
                <td>
      ...
      <input id="contentAwardImageUrl" name="contentAwardImageUrl" type="hidden" runat="server" />
